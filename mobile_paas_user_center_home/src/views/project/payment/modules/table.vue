  ...
  <div>
    <a-table
      ...
      <span slot="action" slot-scope="text, record">
        <template>
          <a @click="handlelCard(record)" v-if="$permissionHandle('projectpayment:handlelCard')">资料卡</a>
          <a-divider type="vertical" />
          <a @click="handleDetail(record)" v-if="$permissionHandle('projectpayment:handleDetail')">详细</a>
        </template>
      </span>
    </a-table>
...
export default {
  ...
  methods: {
    ...
    confirm (id) {
      ...
      this.$emit('deleteMult', this.deleteList)
    },
    _getList () {
      this.$emit('_getList')
    },
