...
export default {
  ...
  methods: {
    ...
    add () {
      this.visible = true
      this.title = '小区常驻车辆库新增'
    },
    update () {
      this.visible = true
      this.$nextTick(() => {
        this.title = '小区常驻车辆库修改'
    ...
    _modify (id) {
      return new Promise((resolve, reject) => {
        modify({ id: id }).then(res => {
          ...
          this.postStatus = parseInt(res.data.isKeyMonitoring)
          this.form.setFieldsValue({
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          return new Promise((resolve, reject) => {
            ...
            this.params.isResident = '1'
            this.params.isKeyMonitoring = this.postStatus
