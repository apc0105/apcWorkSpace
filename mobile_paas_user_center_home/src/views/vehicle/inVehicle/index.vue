    ...
    <a-card>
      <div class="table-page-search-wrapper">
        <a-form :form="form">
          ...
          <a-row :gutter="48">
            <a-col>
              <a-form-item>
                <a-button type="primary" @click="_getList" v-if="$permissionHandle('vehicleIn:_getList')">查询</a-button>
                <a-button type="primary" @click="handleAdd" v-if="$permissionHandle('vehicleIn:handleAdd')">新增</a-button>
                <a-button @click="refresh" v-if="$permissionHandle('vehicleIn:refresh')">重置</a-button>
                <a-button type="danger" @click="deleteMult" v-if="$permissionHandle('vehicleIn:deleteMult')">删除</a-button>
              </a-form-item>
            </a-col>
          </a-row>
        </a-form>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          <template>
            <a @click="handleEdit(record)" v-if="$permissionHandle('vehicleIn:handleEdit')">修改</a>
            <a-divider type="vertical" />
            <a @click="confirm(record.id)" v-if="$permissionHandle('vehicleIn:confirm')">删除</a>
          </template>
        </span>
      </a-table>
